...
def load_system_checkpoint(path: str, map_location="cpu", inference_override: Optional[PrototypeInferenceConfig] = None, strict: bool = True):
    ...
    relaxed_prefixes = []
    if strict and not any(key.startswith("transformer_decoder.attn_residual_queries") for key in model_state_dict):
        relaxed_prefixes.append("transformer_decoder.attn_residual_")
    if strict and not any(key.startswith("slot_decoder.") for key in model_state_dict):
        relaxed_prefixes.extend(("slot_decoder.", "slot_queries."))

    incompatible = system.model.load_state_dict(
        model_state_dict,
        strict=False if relaxed_prefixes else strict,
    )

    if relaxed_prefixes:
        missing = [
            key for key in incompatible.missing_keys
            if not any(key.startswith(prefix) for prefix in relaxed_prefixes)
        ]
